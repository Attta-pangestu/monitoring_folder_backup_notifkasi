    ...
    def generate_zip_checklist(self, file_path: str, analysis: Dict) -> Dict:
        ...
        try:
            ...
            if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
                ...
                checklist['ketersediaan_file']['hasil'] = '[VALID]'

            # Check ukuran file
            file_size = os.path.getsize(file_path)
            if file_size > 1024:  # > 1KB
                checklist['ukuran_file']['status'] = True
                checklist['ukuran_file']['hasil'] = '[VALID]'
            ...
            try:
                ...
                if 0 <= time_diff <= 365:  # Backup dalam 1 tahun terakhir
                    ...
                    checklist['tanggal_backup_valid']['hasil'] = '[VALID]'
                else:
    ...
    def generate_bak_checklist(self, bak_file: str, backup_type: str, analysis_info: Dict = None) -> Dict:
        ...
        try:
            ...
            if bak_size >= min_size_threshold:
                ...
                checklist['ukuran_minimum']['hasil'] = f'[VALID] (≥{size_display})'
            else:
            ...
            if backup_type != 'Unknown':
                ...
                checklist['jenis_backup_dikenal']['hasil'] = '[VALID]'
            ...
            if analysis_info and analysis_info.get('header_valid', False):
                ...
                checklist['header_dapat_dibaca']['hasil'] = '[VALID]'
            ...
            if analysis_info and analysis_info.get('can_restore', False):
                ...
                checklist['format_backup_valid']['hasil'] = '[VALID]'
                checklist['bisa_direstore']['status'] = True
                checklist['bisa_direstore']['hasil'] = '[VALID]'
            ...
            try:
                ...
                if 0 <= time_diff <= 365:
                    ...
                    checklist['tanggal_backup_masuk_akal']['hasil'] = '[VALID]'
                else:
...
"""
    ...
    def send_overdue_notification(self, latest_date: datetime):
        ...
        try:
            subject = "URGENT: Backup System - Backup Terlambat Terdeteksi"
...
"""
    ...
    def send_test_email(self):
        """Kirim email test"""
        try:
            subject = "Monitor Backup Zip - Email Test"
        ...
        <div class="info-box">
            ...
            <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                    ...
                    <td style="padding: 10px; border-bottom: 1px solid #ddd;"><span style="color: #28a745; font-weight: bold;">Berhasil</span></td>
                </tr>
                <tr>
...
"""
        ...
        for backup_type, stats in bak_stats.items():
            if stats['count'] > 0:
                ...
                if backup_type == "BackupStaging":
                    type_display = "Backup Staging (GWScanner)"
                elif backup_type == "BackupVenus":
...
"""
        ...
        body += f"""
            ...
            {'<div class="warning"><strong>Catatan:</strong> PlantwareP3 dikecualikan dari analisis detail sesuai konfigurasi.</div>' if exclude_plantware and bak_stats.get('PlantwareP3', {}).get('count', 0) > 0 else ''}
        </div>

        <div class="summary-box">
            <h3>Detail File per ZIP</h3>
"""
...
"""
        ...
        <div class="summary-box">
            <h3>Checklist Analisis File ZIP</h3>
"""
...
"""
        ...
        <div class="summary-box">
            <h3>Checklist Analisis File BAK</h3>
"""
