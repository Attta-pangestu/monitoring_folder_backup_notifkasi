#!/usr/bin/env python3
"""
Modified version that shows all ZIP files regardless of date
"""
...
class ZipBackupMonitor:
    def __init__(self, root):
        self.root = root
        self.root.title("Monitor Backup Zip - All Dates")
    ...
    def create_gui(self):
        ...
        note_frame = ttk.LabelFrame(main_frame, text="Informasi", padding="10")
        note_frame.grid(row=3, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=5)
        ttk.Label(note_frame, text="Versi ini menampilkan SEMUA file ZIP tanpa filter tanggal",
                 font=('Arial', 10, 'italic')).grid(row=0, column=0, sticky=tk.W)
    ...
    def _scan_files_thread(self):
        ...
        try:
            ...
            self.update_log(f"Ditemukan {len(zip_files)} file ZIP")

            # Analyze ALL files (no date filtering)
            self.analyze_files_threaded(zip_files)
    ...
    def update_summary(self):
        ...
        summary_text = f"""Laporan Ringkasan Backup (Semua Tanggal)
=============================
...
"""
        ...
        body += f"\n\nEmail ini dihasilkan otomatis oleh Monitor Backup Zip v1.0"
