...
# Restore to D: drive to avoid disk space issues
backup_file = r"D:\Gawean Rebinmas\App_Auto_Backup\Backup\BackupStaging 2025-10-04 09;16;30.zip"
print(f"Testing restore to D: drive with: {backup_file}")
...
try:
    ...
    print("Step 3: Attempting restore to D: drive...")
    db_name = "D_Drive_Staging_DB"
    
    # Use D: drive for database files
    db_dir = "D:\\SQLData"
    ...
    success_indicators = ["successfully", "processed", "100 percent"]
    is_success = any(indicator in result.stdout.lower() for indicator in success_indicators)
    
    if result.returncode == 0 and is_success:
        ...
        if db_name in check_result.stdout:
            print("SUCCESS: Database exists and persists!")
            ...
            if connection:
                ...
                print('All tests passed! The database restore system is working correctly.')
                connection.close()
            else:
